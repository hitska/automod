...
function alert(a, do_confirm, confirm_ok_action, confirm_cancel_action) {
      ...
      if (do_confirm) {
              ...
              $("<button class='button alert_button'>"+_("Отменить")+"</button>").click(confirm_cancel_action).click(close).appendTo(div);
...
var selectedstyle = 'Spirech';
var styles = {
	
	'Yotsuba B' : '',
	'Yotsuba' : '/stylesheets/yotsuba.css',
	'Spirech' : '/stylesheets/spirech.css',
	'Lolifox' : '/stylesheets/lolifox.css',
	'Photon' : '/stylesheets/photon.css',
	'Dark' : '/stylesheets/dark.css',
	'Jungle' : '/stylesheets/jungle.css',
	'Spirech Dark' : '/stylesheets/spirech-dark.css',
...
function dopost(form) {
	...
	return (form.elements.body && form.elements['body'].value != "") || (form.elements['file'] && form.elements['file'].value != "") || (form.elements.file_url && form.elements['file_url'].value != "");
}
...
function citeReply(id, with_link) {
	...
	if (document.selection) {
		...
		// двигаем курсор вперед
		textarea.selectionStart = end + ('>>' + id).length + 1;
	...
	$('form#quick-reply textarea#body').focus();
...
function init() {
	init_stylechooser();
		...
		if (document.forms.postcontrols && document.forms.postcontrols.hasOwnProperty('password')) {
...
var youtube_embedding_width = 500;
var youtube_embedding_height = 380;

var vimeo_embedding_width = 500;
var vimeo_embedding_height = 380;

var bitchute_embedding_width = 500;
var bitchute_embedding_height = 380;
...
function actually_load_captcha(provider, extra) {
	...
	if (typeof window.board_name === "undefined" || !window.board_name) {
			var board_name = "technical";
			window.board_name = "technical";
	}


	$(".captcha .captcha_html").css("cursor", "progress");
	let payload = {
		mode: 'get',
		extra: extra,
	...
	if (active_page === "index")
		payload["threadcap"] = true;
	
	$.getJSON(provider, payload, function(json) {

		$(".captcha .captcha_cookie").val(json.cookie);
		$(".captcha .captcha_html").html(json.captchahtml);
		$(".captcha .captcha_html").css("cursor", "pointer");
...
function load_captcha(provider, extra) {
	$(function() {
		...
		if (typeof window.tor_postcaptcha === "undefined") {
			var tor_postcaptcha = false;
		}
		...
		if (window.tor_postcaptcha && window.active_page === "thread" && !(window.location.hostname.endsWith(".onion") || window.location.hostname.endsWith(".i2p"))) {
			$('.captcha .captcha_text, textarea[id="body"]').off("focus");
		...
		$("#quick-reply .captcha .captcha_text").prop("placeholder", _("Капча"));
		...
		$(document).on("ajax_after_post", function() {
			$("#quick-reply .captcha .captcha_text").prop("value", "");
		});
		redo_events(provider, extra);
...
$(function(){
  options_button = $("<a href='javascript:void(0)' class='options_button' title='"+_("Настройки")+"'>["+_("Настройки")+"]</a>");
...
onready(function(){
	...
	var stylesSelect = $('<select class="boardlist-style-select"></select>');
	...
	setTimeout(function() {
		$("div.boardlist:nth-child(1)").append($("<span id='before-styles'>Стиль: </span>").append(stylesSelect));
	}, 10)
	//$("div.boardlist:nth-child(1)").append(stylesSelect).append($("<span id='before-styles'>Стиль: </span>"));



	// $("div.boardlist:nth-child(1)").append(
	// 	$('<div id="style-select" style="float:right;margin-bottom:10px"></div>')
	// 		.text(_('Style: '))
	// 		.append(stylesSelect)
	// 		);
...
var tab = Options.add_tab("general", "home", _("Основное"));

$(function(){
  var stor = $("<div>"+_("Хранилище: ")+"</div>");
  stor.appendTo(tab.content);

  $("<button>"+_("Экспорт")+"</button>").appendTo(stor).on("click", function() {
    var str = JSON.stringify(localStorage);

    $(".output").remove();
    $("<input type='text' class='output'>").appendTo(stor).val(str);
  });
  $("<button>"+_("Импорт")+"</button>").appendTo(stor).on("click", function() {
    var str = prompt(_("Вставьте ваши локальные данные"));
    if (!str) return false;
  ...
  $("<button>"+_("Очистить")+"</button>").appendTo(stor).on("click", function() {
    if (confirm(_("Вы действительно хотите очистить локальное хранилище? Это удалит ваши скрытые треды, просмотренные треды, пароль и многое другое."))) {
      localStorage.clear();
      document.location.reload();
    }
  });
...
$(document).ready(function(){
	...
	var dateformat = (typeof strftime === 'undefined') ? function(t) {
		return zeropad(t.getDate(), 2) + "/" + zeropad(t.getMonth() + 1, 2) + "/" + t.getFullYear().toString().substring(2) +
				" (" + [_("Воскресенье"), _("Понедельник"), _("Вторник"), _("Среда"), _("Четверг"), _("Пятница"), _("Суббота"), _("Воскресенье")][t.getDay()]  + ") " +
				// time
	...
	function timeDifference(current, previous) {
		...
		if (elapsed < msPerMinute) {
			...
			return Math.round(elapsed/msPerMinute) + (Math.round(elapsed/msPerMinute)<=1 ? ' минуту назад':' минут(ы) назад');
		} else if (elapsed < msPerDay ) {
			return Math.round(elapsed/msPerHour ) + (Math.round(elapsed/msPerHour)<=1 ? ' час назад':' часов(а) назад');
		} else if (elapsed < msPerMonth) {
			return Math.round(elapsed/msPerDay) + (Math.round(elapsed/msPerDay)<=1 ? ' день назад':' дней(я) назад');
		} else if (elapsed < msPerYear) {
			return Math.round(elapsed/msPerMonth) + (Math.round(elapsed/msPerMonth)<=1 ? ' месяц назад':' месяцев(а) назад');
		} else {
			return Math.round(elapsed/msPerYear ) + (Math.round(elapsed/msPerYear)<=4 ? ' год(а) назад':' лет назад');
		}
	}
	...
	if (window.Options && Options.get_tab('general') && window.jQuery) {
		var interval_id;
		Options.extend_tab('general', '<label id="show-relative-time"><input type="checkbox">' + _('Показать натуральное время') + '</label>');
...
$(document).ready(function(){
	...
	// Add an update link
	$('.boardlist.bottom').prev().after("<span id='updater'><a href='#' id='update_thread' style='padding-left:10px'>["+_("Обновить")+"]</a> (<input type='checkbox' id='auto_update_status' checked> "+_("Автообновление")+") <span id='update_secs'></span></span>");
		...
		$.ajax({
			...
			success: function(data) {
				...
				if ($('#auto_update_status').is(':checked')) {
					...
					if (loaded_posts > 0)
						$('#update_secs').text(fmt(_("Новых постов: {0}"), [loaded_posts]));
					else
						$('#update_secs').text(_("Нету новых постов"));
				}
			},
			error: function(xhr, status_text, error_text) {
				if (status_text == "error") {
					if (error_text == "Тред не найден") {
						$('#update_secs').text(_("Тред удален или перемещён"));
						$('#auto_update_status').prop('checked', false);
						$('#auto_update_status').prop('disabled', true); // disable updates if thread is deleted
						return;
					} else {
						$('#update_secs').text("Ошибка: "+error_text);
					}
				} else if (status_text) {
					$('#update_secs').text(_("Ошибка: ")+status_text);
				} else {
					$('#update_secs').text(_("Неизвестная ошибка"));
				}
...
var tab = Options.add_tab("user-css", "css3", _("Кастом CSS"));
...
var submit = $("<input type='button' value='"+_("Загрузить свой CSS")+"'>").css({
  position: "absolute",
  height: 25, bottom: 5,
  width: "calc(100% - 10px)",
  left: 5, right: 5
...
var update_textarea = function() {
  if (!localStorage.user_css) {
    textarea.text("/* "+_("Записывайте сюда свой CSS...")+" */\n" +
                  "/* "+_("Если вы хотите сделать перераспределенный стиль, убедитесь\nчто выбрана тема Youtsuba B.")+" */\n" +
                  "/* "+_("Вы можете импортировать CSS из стороннего источника, например:")+" */\n" +
...
var tab = Options.add_tab("user-js", "code", _("Кастом JS"));
...
var submit = $("<input type='button' value='"+_("Обновить кастом JS")+"'>").css({
  position: "absolute",
  height: 25, bottom: 5,
  width: "calc(100% - 10px)",
  left: 5, right: 5
...
var update_textarea = function() {
  if (!localStorage.user_js) {
    textarea.text("/* "+_("Пишите тут JS...")+" */\n" +
                  "/* "+_("Сохраните настройки сайта перед этим\nтак как сайт может стать для вас нерабочим.")+" */\n" +
                  "/* "+_("Вы можете импортировать JS файлы со стороннего сервера, например:")+" */\n" +
...
var favorites = JSON.parse(localStorage.favorites);
Options.add_tab('fav-tab','star',_("Избранное"));
...
$("<span>"+_("Перетащите доски чтобы отсортировать их.")+"</span><br><br>").appendTo(Options.get_tab('fav-tab').content);
...
$(document).ready(function() {
	var force_anon = function() {
		if ($(this).children('a.capcode').length == 0) {
			...
			p.children('span.name').text('Аноним');
	...
	if (window.Options && Options.get_tab('general')) {
		...
		Options.extend_tab("general", "<label id='hide-ids'><input type='checkbox' /> "+_('Скрыть ID')+"</label>");
		Options.extend_tab("general", "<label id='forced-anon'><input type='checkbox' /> "+_('Все анонимы')+"</label>");
	}
	else {
		var s1 = '#hide-ids', s2 = '#forced-anon', e = 'click';
		$('hr:first').before('<div id="hide-ids" style="text-align:right"><a class="unimportant" href="javascript:void(0)">Скрыть ID</a></div>');
		$('hr:first').before('<div id="forced-anon" style="text-align:right"><a class="unimportant" href="javascript:void(0)">-</a></div>');
		$('div#forced-anon a').text(_('Все анонимы')+' (' + (forced_anon ? _('да') : _('нет')) + ')');
	}
	...
	$(s2).on(e, function() {
		...
		if (forced_anon) {
			$('div#forced-anon a').text(_('Все анонимы')+' ('+_('да')+')');
			localStorage.forcedanon = true;
			enable_fa();
		} else {
			$('div#forced-anon a').text(_('Все анонимы')+' ('+_('нет')+')');
...
if (active_page == 'thread' || active_page == 'index') {
	$(document).ready(function(){
		if (window.Options && Options.get_tab('general')) {
			...
			var e = 'change';
			Options.extend_tab("general", "<label id='color-ids'><input type='checkbox' /> "+_('Цветные ID')+"</label>");
		}

		else {
			var selector = '#color-ids';
			var e = 'click';
			$('hr:first').before('<div id="color-ids" style="text-align:right"><a class="unimportant" href="javascript:void(0)">'+_('Цветные IDs')+'</a></div>')
		}
...
watchlist.render = function(reset) {
	...
	JSON.parse(localStorage.watchlist).forEach(function(e, i) {
		...
		'<span>/'+e[0]+'/ - ' +
		'<a href="'+e[3]+'">'+e[1].replace("thread_", _("Тред #"))+'</a>' +
		' ('+e[2]+') </span>' +
		...
		$((active_page == 'ukko') ? 'hr:first' : (active_page == 'catalog') ? 'body>span:first' : 'form[name="post"]').before(
			$('<div id="watchlist">'+
					'<div class="watchlist-controls">'+
						'<span><a id="clearList">['+_('Почистить список')+']</a></span>&nbsp'+
						'<span><a id="clearGhosts">['+_('Почистить несуществующие')+']</a></span>'+
					'</div>'+
					threads.join('')+
...
watchlist.add = function(sel) {
	...
	if (active_page === 'thread') {
		...
		alert('Функциональность не готова к такому типу страницы.');
		return this;
	}
...
$(document).ready(function(){
	...
	$('.boardlist').append(' <span class="watchlist_button">/ <a class="watchlist-toggle" href="#">'+_('Избранное')+'</a></span>');
	//Append a watch thread button after every OP post number.
	$('.op>.intro>.post_no:odd').after(' <a class="watchThread" href="#">'+_('Добавить тред в избранное')+'</a>');
...
(function() {
	...
	var do_css = function() {
		...
		#quick-reply tr td input[name="subject"]{\
			width: 95%;\
			\
		}\
		#quick-reply table {\
	...
	var show_quick_reply = function(){
				...
				if ($td.find('input[type="file"]').length) {
					if ($td.find('input[name="file_url"]').length) {
						...
						if (settings.get('show_remote', false)) {
							...
							$file_url.clone().attr('placeholder', _('Загрузить через ссылку')).appendTo($newRow.find('td'));
		...
		// удаление информации о максимальном размере файлов и тд.
		$postForm.find("ul#post-info").empty();

		//$postForm.find('textarea[name="body"]').removeAttr('id').removeAttr('cols').attr('placeholder', _('Ваше важное сообщение'));
		$postForm.find('textarea[name="body"]').removeAttr('id').removeAttr('cols');
		...
		//$origPostForm.find('textarea[name="body"]').on('change input propertychange', countSymbols);

		// Synchronise body text with original post form
		$origPostForm.find('textarea[name="body"]').on('change input propertychange', function(e) {
			e.preventDefault();
			$postForm.find('textarea[name="body"]').val($(this).val());
		});
		$postForm.find('textarea[name="body"]').on('change input propertychange', function(e) {
			e.preventDefault();
			$origPostForm.find('textarea[name="body"]').val($(this).val());
			// посчет символов для быстрой формы
			countSymbols(e);
		});
...
var update_own = function() {
  ...
  if (posts[board] && posts[board].indexOf(id) !== -1) { // Own post!
    $(this).addClass('you');
    $(this).find('span.name').first().append(' <span class="own_post">'+_('(Вы)')+'</span>');
  }
...
if (active_page == 'catalog') {
	onready(function () {
		...
		$('#catalog_search_button').text('Поиск');
...
function addButton(post) {
	...
	$ele.find('input.delete').after(
		$('<a>', {href: '#', class: 'post-btn', title: 'Опции'}).text('≡')
	);
}
...
cssString =
	...
	'.post-btn {transition: transform 0.1s; width: 15px; text-align: center; font-size: inherit; opacity: 0.8; text-decoration: none; margin: -6px 0px 0px -5px !important; display: inline-block;}\n' +
...
if (active_page === 'thread' || active_page === 'index' || active_page === 'catalog' || active_page === 'ukko') {
	$(document).on('menu_ready', function () {
		...
		function initPostMenu(pageData) {
			...
			var submenu;
			Menu.add_item('filter-menu-hide', _('Скрыть пост'));
			Menu.add_item('filter-menu-unhide', _('Раскрыть пост'));

			submenu = Menu.add_submenu('filter-menu-add', _('Добавить фильтр'));
				submenu.add_item('filter-add-post-plus', _('Пост+'), _('Скрыть пост и ответы'));
				submenu.add_item('filter-add-id', _('ID'));
				submenu.add_item('filter-add-id-plus', _('ID+'), _('Скрыть ID и все ответы'));
				submenu.add_item('filter-add-name', _('Имя'));
				submenu.add_item('filter-add-trip', _('Трипкод'));

			submenu = Menu.add_submenu('filter-menu-remove', _('Удалить фильтр'));
				submenu.add_item('filter-remove-id', _('ID'));
				submenu.add_item('filter-remove-name', _('Имя'));
				submenu.add_item('filter-remove-trip', _('Трипкод'));
				...
				if (rule.regex) {
					try {
						...
						if (e instanceof SyntaxError) {
							console.log(_(`Ваш RegEx (${rule.value}) содержит ошибку синтаксиса. Попробуйте протестировать его прежде чем добавлять.`));
						} else {
							console.log(e);
		...
		function drawFilterList() {
			...
			$ele.append('<tr id="header"><th>Тип</th><th>Правило</th><th>Удалить</th></tr>');
		...
		function initOptionsPanel() {
			if (window.Options && !Options.get_tab('filter')) {
				Options.add_tab('filter', 'list', _('Фильтры'));
				Options.extend_tab('filter',
					'<div id="filter-control">' +
						'<select>' +
							'<option value="name">'+_('Имя')+'</option>' +
							'<option value="trip">'+_('Трипкод')+'</option>' +
							'<option value="sub">'+_('Тема')+'</option>' +
							'<option value="com">'+_('Сообщение')+'</option>' +
						'</select>' +
						'<input type="text">' +
						'<input type="checkbox">' +
						'regex ' +
						'<button id="set-filter">'+_('Добавить')+'</button>' +
						'<button id="clear">'+_('Очистить всё')+'</button>' +
						'<div id="confirm" class="hidden">' +
							_('Это удалит все фильтры, включая скрытые посты.')+' <a id="confirm-y" href="#">'+_('да')+'</a> | <a id="confirm-n" href="#">'+_('отменить')+'</a>' +
						'</div>' +
					'</div>' +
		...
		function init() {
			...
			var pageData = {
				...
				forcedAnon: ($('input[name="name"]').length === 0)  // tests by looking for the Name label on the reply form
			};
...
if ($('#delete-fields #password').length) {
	Menu.add_item("delete_post_menu", _("Удалить пост"));
	// Menu.add_item("delete_file_menu", _("Delete file"));
	Menu.add_item("delete_file_menu_1", _("Удалить файл 1"));
	Menu.add_item("delete_file_menu_2", _("Удалить файл 2"));
	Menu.add_item("delete_file_menu_3", _("Удалить файл 3"));
	Menu.add_item("delete_file_menu_4", _("Удалить файл 4"));
	Menu.onclick(function(e, $buf) {
		...
		if ($ele.hasClass("op")) {
			countFiles = $ele.siblings("div.files").find("div.file").length;
		} else {
			countFiles = $ele.find("div.file").length;
		}
		...
		$buf.find('#delete_post_menu,#delete_file_menu_1,#delete_file_menu_2,#delete_file_menu_3,#delete_file_menu_4').click(function(e) {
			e.preventDefault();

			if ($(this).attr('id').startsWith('delete_file_menu_') && $(this).attr('id').slice(-1) <= countFiles) {
				$('#delete_file')[0].value = $(this).attr('id').slice(-1);

			} else if ($(this).attr('id').startsWith('delete_file_menu_') && $(this).attr('id').slice(-1) > countFiles) {
				alert(_("Неверный номер файла."));
				return;
			}
			 else {
				$('#delete_file')[0].value = 0;
			}
...
$(document).on("ready", function() {
	$("form[name='postcontrols'] input[type='password']").on("focus", function(e) {
		$(e.target).prop("type", "text");
	});
	$("form[name='postcontrols'] input[type='password']").on("blur", function(e) {
		$(e.target).prop("type", "password");
	});
});$('document').ready(function () {
	var autoScroll = localStorage['autoScroll'] ? true : false;
	if (window.Options && Options.get_tab('general')){
		Options.extend_tab('general','<label id=\'autoScroll\'><input type=\'checkbox\' />' + ' К новым постам' + '</label>');
	...
	if (active_page == 'thread') {
		$('span[id="updater"]').children('a').after(' (<input class="auto-scroll" type="checkbox"></input> К новым постам)');
		$('input.auto-scroll').prop('checked', autoScroll);
		$(document).on('new_post', function (e, post) {
...
$(document).ready(function () {
	...
	if (window.Options && Options.get_tab('general')) {
		Options.extend_tab('general', '<label id="file-drag-drop"><input type="checkbox">' + _('Drag and drop') + '</label>');
...
$('<div class="dropzone-wrap" style="display: none;">'+
	'<div class="dropzone" tabindex="0">'+
		'<div class="file-hint">'+_('Выберите/перетащите/вставьте файл сюда')+'</div>'+
...
$(document).on('keypress click', '.dropzone', function (e) {
	e.stopPropagation();
	// тут проблемы в тор-браузерах на телефоне
...
$(document).on('paste', function (e) {
	...
	if (typeof clipboard.items != 'undefined' && clipboard.items.length != 0) {
		...
		alert(_("Не удалось загрузить файл."))
	}
});
...
Math.floor_double = function(num) {
...
var markups = {
		...
		".set-sub": ["[sub]", "[/sub]"],
		".set-greentext": ["> ", ""],
		".set-spoiler": ["[spoiler]", "[/spoiler]"],
		".set-code": ["[code]", "[/code]"],
		".set-quotes": ["«", "»"],
		".set-acronym": ["[acronym]", "[title]Что-то непонятное...[/title][/acronym]"],
}
...
var containerCoef = 0.85;  // 85% от максимального размера окна.
var max_width = window.innerWidth * containerCoef;
var max_height = window.innerHeight * containerCoef;

var minimum_width = 100;  // 100px
var minimum_height = 100;  // 100px
var scaleStep = 0.08;

var hw_coef;


//youtube_embedding_width
//youtube_embedding_height
var auto_embedding = [
	[/https:\/\/(www\.youtube\.com\/watch\?v=|youtu\.be\/)(?<id>[a-zA-Z0-9\-_]{10,12})/is,
	`<iframe class="inpost-frame %id%" width="${youtube_embedding_width}" height="${youtube_embedding_height}" frameborder="0" id="ytplayer" src="https://www.youtube.com/embed/%id%"></iframe>`],

	[/https:\/\/vimeo\.com\/(?<id>[0-9]{8,11})/is,
	`<iframe class="inpost-frame %id%" src="https://player.vimeo.com/video/%id%?embedparameter=value" width="${vimeo_embedding_width}" height="${vimeo_embedding_height}" frameborder="0" allowfullscreen></iframe>`],

	[/https:\/\/www\.bitchute\.com\/video\/(?<id>[A-Za-z0-9]{10,12})\//is,
	`<iframe class="inpost-frame %id%" src="https://www.bitchute.com/embed/%id%/" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture; fullscreen" width="${bitchute_embedding_width}" height="${bitchute_embedding_height}" frameborder="0" allowfullscreen></iframe>`],
];

$(window).on("quick-reply", function(e) {
	setTimeout(function() {
		load_markup_buttons("form#quick-reply");
	}, 0.3);
});
...
$(document).ready(function(e) {
	...
	// показать кнопки разметки
	$(".stylebuttons").before("<br>").css("display", "inline-block");

	// события для кнопок
	//quick-reply
	//form[name='post']
	load_markup_buttons("body > form[name='post']:not(#quick-reply)");

	// добавление эвентов для открытия некоторых ссылок через фрейм
	load_embed_frames($("div.body a"));

	// событие нового поста
	$(window).on("new_post", function(e, post) {
		load_embed_frames($(post).find("div.body").children("a"));

		$(post).find(".post-image").parent().each(function() {
			// обрабатываем только разрешенные медиа
			let ext = $(this).data("extension");
			if (jQuery.inArray(ext, media_extend) != -1) {
				loadImage(this);
			}
		});
	...
	// показать подсчет оставшихся символов
	$("#symbols-left").css("display", "block");
	$("textarea#body").on('change input propertychange', countSymbols);
	countSymbols();
...
// сделал отдельную функцию чтобы было легче обрабатывать посты
function loadImage(target) {
	...
	$target.on("click", function(e) {
		...
		var height = $target.parent("a").data("height") ? $target.parent("a").data("height") : 50;
		var width = $target.parent("a").data("width") ? $target.parent("a").data("width") : 230;
		var extension = $target.parent("a").data("extension");
		var src = $target.parent("a").data("src");

		is_mouse_down = false;

		if (extension === "webm" || extension === "mp4" || extension === "avi") {
			video = $(`<video src="${src}" loop="" controls="" autoplay>Твой браузер не поддерживает HTML5 видео.</video>`)
				.css("height", "100%")
				.css("width", "100%")
				.css("display", "block")[0];
			...
			video.poster = $target.attr("src");

			// у хрома другой размер кнопок
			if ((typeof navigator.userAgent !== "undefined") && navigator.userAgent.includes("Chrome")) {
				$(".imagecontainer").html(video).append($(`<a href="${src}" style="background:transparent;position:absolute;top:0;left:0;width:100%;height:calc(100% - 70px);display:block;" download></a>`));
			} else {
				$(".imagecontainer").html(video).append($(`<a href="${src}" style="background:transparent;position:absolute;top:0;left:0;width:100%;height:calc(100% - 41px);display:block;" download></a>`));
			}

			if (video.readyState == 0) {
				// сохраняем изменение громкости
				video.addEventListener("volumechange", function(e){
					changeSetting("videovolume", video.volume);
				}, false);

				// загружаем из настроек громкость
			...
			audio = $(`<audio src="${src}" loop="" controls="" autoplay>Твой браузер не поддерживает HTML5 видео.</video>`)
				.css("width", "100%")
				.css("height", "100%")
				//.css("padding", "10px")
				.css("display", "block")[0];
			...
			audio.controls = true;
			audio.loop = true;
			audio.muted = (setting("videovolume") == 0);
			audio.volume = setting("videovolume");

			$(".imagecontainer").html(audio).append($(`<a href="${src}" style="background:transparent;position:absolute;top:0;left:0;width:100%;height:calc(100% - 41px);display:block;" download></a>`));


			if (audio.readyState == 0) {
				// сохраняем изменение громкости
				audio.addEventListener("volumechange", function(e){
					changeSetting("videovolume", audio.volume);
				}, false);

				// загружаем из настроек громкость
				audio.addEventListener("loadedmetadata", function(e){
					audio.volume = Math.max(setting("videovolume") - 0.001, 0);
					audio.volume = setting("videovolume");
				}, false);
			} else {
				setTimeout(function() {
		...
		else if (extension === "youtube" || extension === "vimeo") {
			let id = $target.parent("a").data("id");

			// один размер на все видео
			height = 360;
			width = 680;

			// youtube, vimeo
			if (extension === "youtube") {
				$iframe = $(`<iframe style="width: 100% !important; height: 80% !important;" frameborder="0" id="ytplayer" src="https://www.youtube.com/embed/${id}" allowfullscreen></iframe>`);
			} else if (extension === "vimeo") {
				$iframe = $(`<iframe style="width: 100% !important; height: 80% !important; background: #00adef;" src="https://player.vimeo.com/video/${id}?embedparameter=value" frameborder="0" allowfullscreen></iframe>`);
			}

			$(".imagecontainer").html($iframe);
		}
		else {
			$(".imagecontainer").html($(`<img src="${src}" alt="Загрузка...">`));
		}

		// здесь должны быть вычисления положения картинки
		
		// коэфф.
		hw_coef = height/width;

		if(width < minimum_width) {
			width = minimum_width;
			height = width * hw_coef;
		}

		if(height < minimum_height) {
			height = minimum_height;
			width = height / hw_coef;
		}
		...
		$(".imagecontainer").css("height", height + "px").css("width", width + "px");
		$(".imagecontainer").css("left", (window.innerWidth - width)/2 + "px").css("top", (window.innerHeight - height)/2 + "px")
...
function moveHandler(e) {
	// обрабатываем только правый клик
	if (e.which != 1)
		return false;
	...
	if (e.type === "mousedown") {
		...
		// оффсет положения мыши на контейнере
		let shiftX = event.clientX - $(".imagecontainer")[0].getBoundingClientRect().left;
		let shiftY = event.clientY - $(".imagecontainer")[0].getBoundingClientRect().top;

		function moveAt(pageX, pageY) {
			// оффсет на сколько пикселей прокручена страница
			pageX -= getBodyScrollLeft();
			pageY -= getBodyScrollTop();

			$(".imagecontainer").css("left", pageX - shiftX + 'px');
			$(".imagecontainer").css("top", pageY - shiftY + 'px');
		}
		...
		is_mouse_down = false;
		if (e.timeStamp - down_timestamp < 250)  // если нажатие короткое - закрываем плавающий контейнер
			closeContainer();
...
function closeContainer() {
	$(".imagecontainer").css("display", "none").html('');
	$(".imagecontainer").css("top", 10 + "px");
	$(".imagecontainer").css("left", 10 + "px");
	$(document).off("mousemove"); // если контейнер был закрыт кликом за пределами контейнера
}
...
function scrollHandler(e) {
	...
	let height = +($(".imagecontainer").css("height").slice(0, -2));
	let width = +($(".imagecontainer").css("width").slice(0, -2));

	let top = +($(".imagecontainer").css("top").slice(0, -2));
	let left = +($(".imagecontainer").css("left").slice(0, -2));

	// оффсет положения мыши на контейнере до ресайза
	let shiftX = event.clientX - $(".imagecontainer")[0].getBoundingClientRect().left;  // left
	let shiftY = event.clientY - $(".imagecontainer")[0].getBoundingClientRect().top;  // top
	...
	if ( height * delta < minimum_height || width * delta < minimum_width)
		return;
	
	$(".imagecontainer").css("height", height * delta + "px");
	$(".imagecontainer").css("width", width * delta + "px");

	// здесь перемещаем контейнер чтобы указатель оставался на том же месте на картинке
	top = top - (delta * shiftY - shiftY);
	left = left - (delta * shiftX - shiftX);
...
function loadImages(e) {
	...
	// для получения разницы во времени между нажатиями
	var down_timestamp;

	$("body").on("click", function(e) {
		if ($(".imagecontainer").css("display") == "block") {
			closeContainer();
		}
	});

	$(".imagecontainer").on("click mousedown mouseup mouseover mouseout", moveHandler);

	// https://learn.javascript.ru/mousewheel
	// с jquery .on() не работает корректно
	if ("onwheel" in document) {
		$(".imagecontainer")[0].addEventListener("wheel", scrollHandler);
	} else if ("onMozMousePixelScroll" in document) {
		$(".imagecontainer")[0].addEventListener("MozMousePixelScroll", scrollHandler);
	} else if ("onDOMMouseScroll" in document) {
		$(".imagecontainer")[0].addEventListener("DOMMouseScroll", scrollHandler);
	}
	else {
		// mousewheel
		$(".imagecontainer")[0].addEventListener("mousewheel", scrollHandler);
	}
	...
	$(".post-image").parent().each(function() {
		// обрабатываем только разрешенные медиа
		let ext = $(this).data("extension");
		if (jQuery.inArray(ext, media_extend) != -1) {
			loadImage(this);
...
function countSymbols(e) {
	...
	if (e === undefined) {
		// в случае вызова этой функции при загрузке страницы
		var valLen = +$("textarea#body").val().length;
	} else {
		var valLen = +$(e.currentTarget).val().length;
	}
	
	// Без явного указания #quick-reply не работает
	if (max_body - valLen < 0) {
		$("#symbols-left, #quick-reply #symbols-left").text(`Превышено на ${-(+max_body - valLen)} символа(ов)`);
		$("#symbols-left").each(function(a) {
			console.log(a);
		})
	} else if (max_body - valLen == 0) {
		$("#symbols-left, #quick-reply #symbols-left").text("Достигнут лимит символов.");
	} else {
		$("#symbols-left, #quick-reply #symbols-left").text(max_body - valLen);
	}
}



function load_embed_frames(targets) {
	targets.each(function(index) {
		for (i in auto_embedding) {
			let regex = auto_embedding[i][0];
			let frame = auto_embedding[i][1];
			if (regex.test(this)) {
				load_frame(this, regex, frame);
			}
		}
	});
}
...
function load_frame(link, regex, frame) {
	var id = link.href.match(regex.source).groups.id;
	frame = frame.replace(new RegExp("%id%", "g"), id);
	// показываем кнопку раскрытия фрейма
	//$(link).append($(`<button class="add-frame-button" data-active="0" title="Раскрыть фрейм в посте">[Раскрыть]</button>`));
	$(link).append($(`<a href="javascript:void(0)" class="add-frame-button" data-active="0" title="Раскрыть фрейм в посте">[Раскрыть]</a>`));
	
	$(link).children(".add-frame-button").on("click", function(e){
		e.preventDefault();
		
		// если фрейм добавлен
		if ($(e.target).attr(`data-active`) == "1") {
			$(e.target).parent("a").next(".inpost-frame").detach();
			$(e.target)
				.attr("data-active", "0")
				.text("[Раскрыть]");
		
		} else { // если фрейм не добавлен
			$(e.target).parent().after($(frame));
			// пауза чтобы элемент прогрузился
			setTimeout(function(){
				$(e.target).attr("data-active", "1").text("[Закрыть]");
			}, 0.2)
		}
	});
}
...
function load_markup_buttons(form_selector) {
	form_selector += " ";

	for (selector in markups) {
		$(form_selector + selector).on("click", function(e) {
			e.preventDefault();

			let target = "." + $(e.currentTarget).attr("class");
			
			let tag1 = markups[target][0];
			let tag2 = markups[target][1];
			let body = $(form_selector + "textarea[name='body']")[0];
			...
			if (document.selection)	{
				...
				let len = body.value.length;
				let start = body.selectionStart;
				let end = body.selectionEnd;
				let scrollTop = body.scrollTop;
				let scrollLeft = body.scrollLeft;
				let selection = body.value.substring(start, end);
				let rep = tag1 + selection + tag2;

				body.value = body.value.substring(0,start) + rep + body.value.substring(end,len);
				$(body).trigger(jQuery.Event("change"));
				body.scrollTop = scrollTop;
				body.scrollLeft = scrollLeft;
				body.focus();
				body.setSelectionRange(start+tag1.length, end+tag1.length);
			}
		});
	}
}
...
// let dt = new Date();
// if ((dt.getMonth() === 3 && dt.getDate() === 1 && active_page === "thread" && board_name === "b") || 0) {
// 	var sound = $('<audio style="display:none" preload="auto" controls src="/static/btn_1april.mp3"></audio>')[0];
// 	$(document).ready(function(){
// 		$("body").append($(sound));
// 		$("input[type='submit']")
// 		.on("focus hover", function(e){
// 			sound.currentTime = 0;
// 			sound.play();
// 		})
// 		.on("blur", function(e) {
